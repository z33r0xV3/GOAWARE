...
if isfile('goaware/games/'..vape.Place..'.lua') then
	runChunk(readfile('goaware/games/'..vape.Place..'.lua'), 'bridge duel')
else
	if not shared.GoAwareDeveloper then
		...
		if suc and res and res ~= '' and res ~= '404: Not Found' then
			runChunk(downloadFile('goaware/games/'..vape.Place..'.lua'), 'bridge duel')
		else
			error('GoAware game source '..tostring(vape.Place)..' was not found: '..tostring(res))
		end
	end
end
